...
namespace zzzJs
{
    ...
    JSClass *GetClass(JSContext *cx, JSObject *obj, const char* className);
	...
	// 指定したプロパティーが存在する時に、プロパティー値を取得。
	// 存在しない場合は、JS_FALSEを返す。
    JSBool GetPropertyWhenPresent(JSContext *cx, 
                         JSObject *obj, 
                         const wxString &propertyName, 
                         jsval *propertyValue);
};
