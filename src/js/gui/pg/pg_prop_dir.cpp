...
//	DirProperty
//--------------------------------------------------------
WXJS_INIT_CLASS(DirProperty, "DirProperty", 0)
	

WXJS_BEGIN_DESTRUCTOR(wxDirProperty, DirProperty)

WXJS_END_DESTRUCTOR

WXJS_BEGIN_CONSTRUCTOR(wxDirProperty, DirProperty)
	...
	wxDirProperty *p = new wxDirProperty(label, name, value);
	SetPrivate(cx, obj, p);
	return p;
...
WXJS_BEGIN_GET_SET_STR_PROP(wxDirProperty, DirProperty)
	DirProperty::SetStringProperty(p, cx,  obj, propertyName, vp);
	return true;
