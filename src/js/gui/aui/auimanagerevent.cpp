 ...
 * </class>
 */
WXJS_INIT_CLASS(AuiManagerEvent, "ManagerEvent", 0)
 ...
 * </properties>
 */
WXJS_BEGIN_PROPERTY_MAP(AuiManagerEvent)
	WXJS_PROPERTY(P_Button, "button")
WXJS_END_PROPERTY_MAP()


WXJS_BEGIN_GET_PROP(PrivAuiManagerEvent, AuiManagerEvent)
    wxAuiManagerEvent *event = p->GetEvent();
	switch(id)
	{
	case P_Button:
...
WXJS_BEGIN_SET_PROP(PrivAuiManagerEvent, AuiManagerEvent)
    wxAuiManagerEvent *event = p->GetEvent();
	if ( id == P_Button )
	{
		int val = 0;
