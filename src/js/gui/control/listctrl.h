...
namespace zzzJs
{
    namespace gui
    {
        ...
        public:
            ...
            static int wxCALLBACK SortFn(wxIntPtr item1, wxIntPtr item2, wxIntPtr data);
            // Fn's for virtual list controls.
            wxString OnGetItemText(long item, long column) const;
            int OnGetItemImage(long item) const;
            wxListItemAttr *OnGetItemAttr(long item) const;
			
			WXJS_DECLARE_ADD_PROP(wxListCtrl)
			WXJS_DECLARE_DEL_PROP(wxListCtrl)
			WXJS_DECLARE_GET_PROP(wxListCtrl)
			WXJS_DECLARE_SET_PROP(wxListCtrl)
			WXJS_DECLARE_SET_STR_PROP(wxListCtrl)

            /**
             * Callback for when a wxListCtrl object is created
             */
			WXJS_DECLARE_CONSTRUCTOR(wxListCtrl)
            WXJS_DECLARE_DESTRUCTOR(wxListCtrl)
