...
#include "js/gui/control/listbook.h"
 ...
 * <module>gui</module>
 * <class name="wxListbook" prototype="@wxWindow">
 *  This is the prototype for a control or 'widget'.
 *  A control is generally a small window which processes user input
 *  and/or displays one or more item of data.
 * </class>
 */
ZZZJS_INIT_CLASS(Listbook, "Listbook", 0)
void Listbook::InitClass(JSContext* cx,
								JSObject* obj,
								JSObject* proto)
{
	ListbookEventHandler::InitConnectEventMap();
}

	
ZZZJS_BEGIN_GET_SET_STR_PROP(wxListbook, Listbook)
{
...
ZZZJS_BEGIN_METHOD_MAP(Listbook)
  WXJS_METHOD("create", create, 1)
ZZZJS_END_METHOD_MAP()

ZZZJS_BEGIN_DESTRUCTOR(wxListbook, Listbook)
{
...
ZZZJS_BEGIN_O2I(wxListbook, Listbook)
{
	
	pRet = Listbook::GetPrivate(cx, obj);
	if( pRet != NULL ) {
		return pRet;
	}
	/*
	...
	*/
	...
	return new wxListbook(parent, id, *(apPoint.get()), *(apSize.get()), style, name);
}
ZZZJS_END_O2I
...
JSBool Listbook::create(JSContext *cx, unsigned argc, JS::Value *vp)
{
...
JSBool Listbook::_innerCreate(JSContext *cx, JSObject *obj, unsigned argc, JS::Value *vp,jsval *rval)
{
ZZZJS_FUNC_ENT
	JS::CallArgs argv = CallArgsFromVp(argc, vp);
	wxListbook *p = GetPrivate(cx, obj);
	if ( p == NULL )
		return JS_FALSE;
	...
	*/
	...
	{
		zzzJsPtr<wxListbook> apNotebook;

		if( apNotebook.set( 
			Listbook::getObject2Instance(cx, argv[argIndex].toObjectOrNull() , apNotebook.bAllocation) ) == true 
		) {
			delete p;
...
WXJS_INIT_EVENT_MAP(wxListbook)
const wxString WXJS_PAGE_CHANGED_EVENT = wxT("onPageChanged");
const wxString WXJS_PAGE_CHANGING_EVENT = wxT("onPageChanging");



void ListbookEventHandler::OnPageChanged(wxBookCtrlEvent &event)
{
	PrivBookCtrlEvent::Fire<BookCtrlEvent>(event, WXJS_PAGE_CHANGED_EVENT);
}

void ListbookEventHandler::OnPageChanging(wxBookCtrlEvent &event)
{
	PrivBookCtrlEvent::Fire<BookCtrlEvent>(event, WXJS_PAGE_CHANGING_EVENT);
}


void ListbookEventHandler::ConnectNotebookPageChanged(wxListbook *p, bool connect)
{
	if ( connect )
	{
		p->Connect( wxEVT_LISTBOOK_PAGE_CHANGED ,
				wxBookCtrlEventHandler(ListbookEventHandler::OnPageChanged));
	}
	else
	{
		p->Disconnect( wxEVT_LISTBOOK_PAGE_CHANGED ,
				wxBookCtrlEventHandler(ListbookEventHandler::OnPageChanged));
	}
}

void ListbookEventHandler::ConnectNotebookPageChanging(wxListbook *p, bool connect)
{
	if ( connect )
	{
		p->Connect(wxEVT_LISTBOOK_PAGE_CHANGING ,
				wxBookCtrlEventHandler(ListbookEventHandler::OnPageChanging));
	}
	else
	{
		p->Disconnect(wxEVT_LISTBOOK_PAGE_CHANGING ,
				wxBookCtrlEventHandler(ListbookEventHandler::OnPageChanging));
	}
}

void ListbookEventHandler::InitConnectEventMap()
{
