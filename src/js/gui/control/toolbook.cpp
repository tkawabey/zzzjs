 ...
 * <module>gui</module>
 * <class name="wxToolbook" prototype="@wxWindow">
 *  This is the prototype for a control or 'widget'.
 *  A control is generally a small window which processes user input
 *  and/or displays one or more item of data.
 * </class>
 */
ZZZJS_INIT_CLASS(Toolbook, "Toolbook", 0)
void Toolbook::InitClass(JSContext* cx,
								JSObject* obj,
								JSObject* proto)
{
	ToolbookEventHandler::InitConnectEventMap();
}

	
ZZZJS_BEGIN_GET_SET_STR_PROP(wxToolbook, Toolbook)
{
...
ZZZJS_BEGIN_METHOD_MAP(Toolbook)
  WXJS_METHOD("create", create, 1)
ZZZJS_END_METHOD_MAP()

ZZZJS_BEGIN_DESTRUCTOR(wxToolbook, Toolbook)
{
...
ZZZJS_BEGIN_O2I(wxToolbook, Toolbook)
{
	
	pRet = Toolbook::GetPrivate(cx, obj);
	if( pRet != NULL ) {
		return pRet;
	}
	/*
	...
	*/
	...
	return new wxToolbook(parent, id, *(apPoint.get()), *(apSize.get()), style, name);
}
ZZZJS_END_O2I
...
JSBool Toolbook::create(JSContext *cx, unsigned argc, JS::Value *vp)
{
...
JSBool Toolbook::_innerCreate(JSContext *cx, JSObject *obj, unsigned argc, JS::Value *vp,jsval *rval)
{
ZZZJS_FUNC_ENT
	JS::CallArgs argv = CallArgsFromVp(argc, vp);
	wxToolbook *p = GetPrivate(cx, obj);
	if ( p == NULL )
		return JS_FALSE;
	...
	*/
	...
	{
		zzzJsPtr<wxToolbook> apNotebook;

		if( apNotebook.set( 
			Toolbook::getObject2Instance(cx, argv[argIndex].toObjectOrNull() , apNotebook.bAllocation) ) == true 
		) {
			delete p;
...
WXJS_INIT_EVENT_MAP(wxToolbook)
const wxString WXJS_PAGE_CHANGED_EVENT = wxT("onPageChanged");
const wxString WXJS_PAGE_CHANGING_EVENT = wxT("onPageChanging");



void ToolbookEventHandler::OnPageChanged(wxBookCtrlEvent &event)
{
	PrivBookCtrlEvent::Fire<BookCtrlEvent>(event, WXJS_PAGE_CHANGED_EVENT);
}

void ToolbookEventHandler::OnPageChanging(wxBookCtrlEvent &event)
{
	PrivBookCtrlEvent::Fire<BookCtrlEvent>(event, WXJS_PAGE_CHANGING_EVENT);
}


void ToolbookEventHandler::ConnectNotebookPageChanged(wxToolbook *p, bool connect)
{
	if ( connect )
	{
		p->Connect(wxEVT_TOOLBOOK_PAGE_CHANGED ,
				wxBookCtrlEventHandler(ToolbookEventHandler::OnPageChanged));
	}
	else
	{
		p->Disconnect(wxEVT_TOOLBOOK_PAGE_CHANGED ,
				wxBookCtrlEventHandler(ToolbookEventHandler::OnPageChanged));
	}
}

void ToolbookEventHandler::ConnectNotebookPageChanging(wxToolbook *p, bool connect)
{
	if ( connect )
	{
		p->Connect(wxEVT_TOOLBOOK_PAGE_CHANGING ,
				wxBookCtrlEventHandler(ToolbookEventHandler::OnPageChanging));
	}
	else
	{
		p->Disconnect(wxEVT_TOOLBOOK_PAGE_CHANGING ,
				wxBookCtrlEventHandler(ToolbookEventHandler::OnPageChanging));
	}
}

void ToolbookEventHandler::InitConnectEventMap()
{
